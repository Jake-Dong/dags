...
def bibio_citaion_DB():
    ...
    yesterday = datetime.today() - timedelta(8) # 매일 매일 어제의 데이터(태이블)를 가지고 와야 할때
    ...
    for date in dt_list:
        ...
        try:
            ...
            cur = conn.cursor()
            sql = "SELECT DISTINCT * FROM pub_num_data_{}".format(
                date)  # date 날짜에 있는 pub_info 를 중복제거해서 가져와야함.(cpc,ipc 별로 가지고 왔기 때문에 중복되는 기술분류가 있을수 있음.)
            cur.execute(sql)
            conn.commit()
