...
def pub_num_DB():
    ...
    for cql1 in cpc_lv1_list:
        print(cql1)
        print('lv1 시작')
        # try,exept,else 로 해당 페이지가 없거나 오류가있는 페이지라면 pass된다.
        # 각 cql 조건을 주었을대 2000개의 범위가 넘는지 아닌지 확인.
        try:
            ...
            for total_result1 in roots.iter('{http://ops.epo.org}biblio-search'):
                ...
                # 만역 total 이 2000 이하라면 수행하는 코드
                if total_result_count1 < 2000:
                    # OPS rainge 를 설정하기 위한 설정 입니다.
                    # total count 를 100 으로 나누고 소수점을 지우기 위해서 int 형으로 바꾸어준다. 반내림의 경우가 있을수있기 때문에 +1 을 해주어 num_list 의 인덱스값으로 rainge 를 설정한다.
                    ...
                    for begin_num, end_num in zip(lv1_begin_num_list, lv1_end_num_list):
                        response = client.published_data_search(
                            ...
                            , range_end=end_num
                            # constituents 에는 biblio,full-cycle 등이 들어갈수있다 자세한 내용은 OPS 매뉴얼 47쪽 부터 참고.
                            , constituents=['biblio']
                        )
                        xmlStr = response.text
                        roots = ET.fromstring(xmlStr)
                        # publication info 출력
                        # xml 파싱기법중 attrib.get 을사용하여 파싱한다.
